...
def IOU(label_stack, output_stack, num_classes=4, bg_weight=0.05, smooth=1):
    ...
    for i in range(len(label_list)):
        ...
        for c in range(num_classes):
            iou_class = (intersect_per_class[c] * smooth) / (label_occurence_per_class[c] + output_occurence_per_class[c] - intersect_per_class[c] + smooth)
            # weight = (len(label_f)-label_occurence_per_class[c]) / len(label_f)
            weighted_iou += iou_class * weights[c]
            ...
            print(f"              IoU_class: {iou_class} | Weight: {weights[c]}")

        total_iou += weighted_iou
...
def running_model(pretrained=False, num_classes=4, loadingfile="weights.h5"):
    ...
    for i_batch, batch in enumerate(dataloading.test_dataloader):
        ...
        total_iou += IOU(batch["label"], output.detach())
    normalized_masks = torch.nn.functional.softmax(output, dim=1)

    iou = total_iou/len(dataloading.test_slicedata)
    print("Evaluation Dice:", iou)
...
def main():
    ...
    if trained is False:
        learningrates = [0.001]
        training_model(pretrained=True, learning_rate=learningrates, batch_size=8, num_epochs=2, test_size=0.2, savingfile=savingfile)
